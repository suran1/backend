...
function wantWords (list, vowels, consonants, forbidLetters) {
    ...
    for (var i = 0; i < list.length; i++) {
        var forbidFlag = 'false';
        arrayList = list[i].split('');
        console.log(list[i].match(/[aeiou]/gi).length, vowels);
        console.log('length of consonants array: ', (list[i].match(/[^aeiou]+/gi).length), ' max consonants: ', consonants);
        console.log(arrayList.includes(forbidLetters), forbidLetters, arrayList);
        
        if ((list[i].match(/[aeiou]/gi).length === vowels) && (!list[i].includes(forbidLetters)) && (list[i].match(/[^aeiou]/gi).length === consonants))  {
            answers.push(list[i]);
        }
    }

   
    var example = ['s', 't', 'r', 'e', 'n', 'g', 't','h'];
    if (example.includes(forbidLetters))
        console.log('has forbidden letters');
